...
# ---------------------------------------------------------------------------
### Verify variables inside-frequency
''' from collections import Counter

strataid = []
dsetid = []     # DatasetID       (same-fleet datasets)
fleetid = []    # FleetID         
geargr = []     # GearGroupeCode  (PS, LL, etc.)
gear = []       # GearCode        (PSG, LLFB, etc.)
filetype = []   # FileTypeCode    (Off Repr., WG-EST Weight estimated, P-SAMP Port sampling)
year = []       # Year
timeid = []     # TimePeriodID    (1-12 month, 13-16 season (quarter), 17 annual)
square = []     # SquareTypeCode  (1x1, 5x5, 10x10)
quadid = []     # QuadID          (1 NE, 2 SE, 3 SW, 4 NW)

schooltype = [] # SchoolTypeCode  (n/a, FAD Fish Aggregate Device, FSC free schools)
efftype = []    # Eff1Type        (D.FISH days fishing, NO.HOOKS, no. of hooks, etc.)
efftype2 = []   # Eff2Type  
dsettype = []   # DSetTypeID      (nw no. and weight, n. no., .w weight, ..)
catchunit = []  # CatchUnit       (kg, nr, ..)
...
for i in fa[1:]:
    ...
    square.append(i[8])
    quadid.append(i[9])
    schooltype.append(i[12])
    efftype.append(i[14])
    efftype2.append(i[16])
    dsettype.append(i[17])
    catchunit.append(i[18])
    BFT.append(i[19])
    ALB.append(i[20])
    YFT.append(i[21])
    BET.append(i[22])
...
count = Counter(strataid)
print('StrataID', count)

count = Counter(dsetid)
print('DataSetID', count)

count = Counter(fleetid)
print('FleetID', count)

count = Counter(geargr)
print('GearGroupe', count)

count = Counter(gear)
print('Gear', count)

count = Counter(filetype)
print('FileType', count)

count = Counter(year)
print('Year', count)

count = Counter(timeid)
print('TimePeriod', count)

count = Counter(square)
print('Square', count)

count = Counter(quadid)
print('QuadID', count)

count = Counter(schooltype)
print('SchoolType', count)

count = Counter(efftype)
print('EffType', count)

count = Counter(efftype2)
print('EffType2', count)

count = Counter(dsettype)
print('DSetType', count)

count = Counter(catchunit)
print('CatchUnit', count) '''
